...
import { technicalProfiles } from "@/lib/technical-data";
...
export default async function ProductPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  const technical = technicalProfiles[product.slug];
  ...
  return <>
    ...
    <section id="how-to-use" className="product-section"><div className="section-heading"><span className="section-index">Application guidance</span><h2>{applicationHeadings[product.slug] ?? `How to use ${product.name} correctly.`}</h2></div><div className="instruction-grid"><div><h3>Surface preparation</h3><ol>{product.preparation.map((item) => <li key={item}>{item}</li>)}</ol></div><div><h3>Application method</h3><ol>{product.applicationSteps.map((item) => <li key={item}>{item}</li>)}</ol></div><div><h3>Usage considerations</h3><ul>{product.limitations.map((item) => <li key={item}>{item}</li>)}</ul></div></div></section>
    <section id="technical" className="product-section technical-panel"><div><span className="section-index">Technical information</span><h2>{product.name} specification.</h2><p>Values below are taken from the current official technical data sheet. Actual performance depends on substrate, preparation and site conditions.</p>{product.tdsUrl?<TdsGate product={product.slug} name={product.name}/>:product.sourceUrl&&<a className="button gold" href={product.sourceUrl} target="_blank" rel="noreferrer">View official product source ↗</a>}</div><dl><div><dt>Chemistry</dt><dd>{product.chemistry}</dd></div><div><dt>Pack sizes</dt><dd>{product.packSizes.join(", ")}</dd></div><div><dt>Colours</dt><dd>{product.colours.join(", ")}</dd></div>{(technical?.shelfLife || product.shelfLife) && <div><dt>Shelf life</dt><dd>{technical?.shelfLife || product.shelfLife}</dd></div>}{(technical?.storage || product.storage) && <div><dt>Storage</dt><dd>{technical?.storage || product.storage}</dd></div>}</dl></section>
    {technical && <section className="product-section technical-detail-section"><div className="section-heading"><span className="section-index">Tested performance</span><h2>Technical data.</h2></div><div className="technical-data-table" role="table" aria-label={`${product.name} technical data`}><div className="technical-data-row technical-data-head" role="row"><span role="columnheader">Property</span><span role="columnheader">Specification</span><span role="columnheader">Testing standard</span></div>{technical.data.map((item) => <div className="technical-data-row" role="row" key={item.label}><strong role="cell">{item.label}</strong><span role="cell">{item.value}</span><span role="cell">{item.standard || "—"}</span></div>)}</div>{technical.data.length<4?<p className="technical-source-note">Only values clearly verified from the supplied TDS are shown. Refer to the current official document or contact technical support for additional properties.</p>:null}{technical.certifications?.length ? <div className="technical-note"><h3>Certifications and standards</h3><ul>{technical.certifications.map((item) => <li key={item}>{item}</li>)}</ul><p className="technical-source-note">Claims shown are reproduced from the supplied TDS. Request the supporting certificate or test report before specification.</p></div> : null}{technical.coverage ? <div className="technical-note"><h3>Coverage</h3><p>{technical.coverage}</p></div> : null}{technical.handling?.length ? <div className="technical-note"><h3>Application controls</h3><ul>{technical.handling.map((item) => <li key={item}>{item}</li>)}</ul></div> : null}<div className="technical-note technical-warning"><h3>Restrictions on use</h3><ul>{technical.restrictions.map((item) => <li key={item}>{item}</li>)}</ul></div></section>}
